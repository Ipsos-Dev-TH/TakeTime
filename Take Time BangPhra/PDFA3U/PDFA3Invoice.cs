...
namespace ECertificateAPI
{
    class PDFA3Invoice
    {
            ...
            string xmlVersion, string documentID, string documentOID, string outputPath, string documentType)
        {
            ...
            ICC_Profile icc = ICC_Profile.GetInstance(AppCfg.Get("BaseFolderPath").ToString()+"\\Resources\\sRGB Color Space Profile.icm");
            ...
            string stringExchangeXMP = File.ReadAllText(AppCfg.Get("BaseFolderPath").ToString()+"\\Resource\\EDocument_PDFAExtensionSchema.xml");
