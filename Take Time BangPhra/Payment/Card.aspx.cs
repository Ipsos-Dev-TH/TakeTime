    ...
    public partial class Card : System.Web.UI.Page
    {
        ...
        private void RenderPage()
        {
            ...
            if (IsHoldMode)
            {
                ...
                // โหมดเงินประกันเป็นโอน/เงินสด หรือเกตเวย์ที่ใช้อยู่ (PaySo) กันวงเงินไม่ได้
                // → ลิงก์กันวงเงินเก่าใช้ไม่ได้แล้ว ไม่แสดงฟอร์มบัตรให้ลูกค้า
                if (!holds.IsCardHoldAvailable)
                {
                    Fail("ที่พักรับเงินประกันโดยการโอนเข้าบัญชีแล้ว (ไม่ใช้การกันวงเงินบัตร) — กรุณาติดต่อเจ้าหน้าที่");
                    return;
                }
