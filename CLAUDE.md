...
#### JE mutability (AccountingService.UpdateJournalEntryAsync — verified มิ.ย. 2026)
`PUT /api/companies/{cid}/accounting/journals/{entryId}` (`UpdateJournalEntryRequest`:
EntryDate?/Description?/Reference?/Lines? — Lines != null = แทนที่ทั้งชุด) ใช้นโยบาย
**operator-override: แก้ได้ทุกสถานะรวม Posted** เพื่อตามแก้ตัวเลข/ผังบัญชีที่บันทึกผิด
**เงื่อนไข:** (1) งวดบัญชี (FiscalPeriod) ต้องยัง **Open** — งวด Closed → 400; (2) **ห้ามแก้ JE
ที่ IsAutoGenerated** (JE ที่ NextAcc auto-post จากเอกสาร Receipt/Invoice/Expense/PV) → เอกสาร
พวกนั้นแก้ได้ทางเดียวคือ void เอกสาร + สร้างใหม่ (TakeTime: ปุ่ม Retry บนคิว COMPLETED
CREATE_RECEIPT_DOCUMENT = `RepostReceiptWithCurrentLogic` ทำ void→recreate อัตโนมัติ; คิว
COMPLETED ประเภทอื่นถูกบล็อกกัน double-post). ถ้าต้องการ audit trail เต็มใช้ Correct
(reversal + draft ใหม่) ฝั่ง NextAcc. Client: `UpdateJournalEntryAsync(entryId, request)`.
