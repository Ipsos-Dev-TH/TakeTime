...
> `ProcessVoidReceipt` กลับ `{receipt}-DEPVATFIX` ตัวจริงตอน void เสมอ (idempotent, ไม่มี = ข้าม) →
> void→recreate ปลอดภัยอัตโนมัติ. หลัง deploy d7ee4d3 auto-fix ฝั่งเราจะไม่มีอะไรให้ทำ (stuck=0).
...
## คำขอ NextAcc: หัวเอกสาร "ใบกำกับภาษี/ใบเสร็จรับเงิน" (TaxInvoice ที่รับชำระเต็มแล้ว)

**อาการ:** เช็คเอาท์/รับชำระเต็ม → ลูกค้ามีเลขภาษี 13 หลัก → TakeTime สร้าง **TaxInvoice (type 4)**
→ NextAcc render หัวเอกสารว่า **"ใบกำกับภาษี"** อย่างเดียว. แต่เป็นการขายรับเงินสด (รับชำระวันเดียวกัน)
→ หัวเอกสารมาตรฐานไทยควรเป็น **"ใบกำกับภาษี/ใบเสร็จรับเงิน"** (เอกสารใบเดียวทำหน้าที่ทั้งใบกำกับภาษี
และใบเสร็จรับเงิน — cash sale).

**สาเหตุเชิงออกแบบ:** TakeTime สร้าง TaxInvoice เป็น **เอกสารเปิดลูกหนี้ (AR, PaymentDate/PaymentAccountId
= null)** แล้วปิดด้วย payment แยก (`SettleReceiptInNextAcc`) — เพื่อให้หักมัดจำ/GL ตรง (กันเงินสดเบิล).
NextAcc จึงมองว่าเป็นใบกำกับ AR → หัว = "ใบกำกับภาษี". การรับเงินอยู่คนละ transaction (payment ที่ปิด AR).

**ไม่มี field ฝั่ง TakeTime ควบคุมหัวเอกสาร** — `CreateDocumentRequest` ไม่มี title/header/type-label
(หัวมาจาก `DocumentType` ล้วน). ⟹ ต้องแก้ที่ **NextAcc PDF template/logic**.

**คำขอ (เลือกอย่างใดอย่างหนึ่ง):**
1. **(แนะนำ)** NextAcc render **TaxInvoice ที่ `BalanceDue = 0` (ชำระครบแล้ว)** เป็นหัว
   **"ใบกำกับภาษี/ใบเสร็จรับเงิน"**; ที่ยังค้างชำระ (credit) คงเป็น "ใบกำกับภาษี". NextAcc มีข้อมูล
   การชำระอยู่แล้ว (payment ที่ปิด AR) → ตัดสินจาก BalanceDue ได้ทันที ไม่ต้องแก้ฝั่ง TakeTime.
2. หรือเพิ่ม field ใน `CreateDocumentRequest` ให้ผู้สร้างเลือกหัว (เช่น `IssueAsTaxReceipt: true` /
   `TitleOverride`) → TakeTime จะส่ง true สำหรับใบเช็คเอาท์/รับชำระเต็มที่มีเลขภาษีครบ.

**หมายเหตุ:** ใบที่ลูกค้า**ไม่มีเลขภาษี** ยังต้องเป็น **"ใบเสร็จรับเงิน"** (Receipt type 3) ตามเดิม —
ออกใบกำกับภาษีไม่ได้ (§86/4). คำขอนี้กระทบเฉพาะใบที่ออกเป็น TaxInvoice (มีเลขภาษีครบ) อยู่แล้ว.

---

## Bug NextAcc: ใบเสร็จรับชำระโชว์ JE preview ผิดบัญชี (ไม่ resolve OverridePaymentAccountId)

**อาการ (REC-20260718-0011, จ่าย BankTransfer เข้ากสิกร):** ท้ายใบเสร็จรับชำระ block
"การบันทึกบัญชี (ประมาณการ — ก่อนอนุมัติ)" แสดง **"Dr 11110 เงินสด 6,400"** ทั้งที่ payment ถูกบันทึก
ด้วย `OverridePaymentAccountId` = เงินฝากกสิกร (11122-001) และ **JE จริงที่โพสต์** (RV-202607-0148
บนใบกำกับ TIV-20260718-0002 ผ่าน net-logic) ลง **Dr 11122-001** ถูกต้อง. เช่นเดียวกับ REC-0010
(ตัดมัดจำ, override 21510) ที่ preview โชว์ Dr 11110 แทน 21510.

**ผล:** GL ถูก แต่**เอกสารที่ลูกค้า/บัญชีเห็นบอกบัญชีผิด** — ผู้ใช้เข้าใจว่ารับเงินสดทั้งที่รับโอน
(ตรวจนับเงินสด/กระทบยอดธนาคารจะงง). ฝั่ง TakeTime กันเพิ่มแล้ว: post-settle verify อ่าน JE จริง
เทียบบัญชีแหล่งเงินที่สั่ง → log ⚠⚠ ถ้าไม่ตรง (พิสูจน์จาก GL ไม่ใช่ preview).

**ขอแก้ (อย่างใดอย่างหนึ่ง):**
1. Preview บนใบเสร็จรับชำระ resolve `Payment.OverridePaymentAccountId` (และ override 21510 ของ
   deposit-cut payment) แทนการ map จาก PaymentMethod default.
2. หรือดีกว่า: เมื่อ payment ถูก fold เข้า net JE ของเอกสารแล้ว (net-logic) — **ไม่ต้องแสดง block
   "ประมาณการ" บนใบเสร็จเลย** (JE จริงอยู่บนใบกำกับ; ประมาณการที่ไม่มีวันโพสต์ = ชวนเข้าใจผิด)
   หรือแสดง JE จริงที่ fold แล้วแทน.

(หมายเหตุ: ใบเสร็จรับชำระพวกนี้จะหมดไปเองเมื่อ cash-sale single-doc + deposit fields ใช้เต็มรูป —
แต่เอกสารเก่า + เคสที่ยังใช้ settle แยกยังโชว์ผิดอยู่ จึงควรแก้ preview ด้วย)
